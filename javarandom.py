#This belongs to https://github.com/MostAwesomeDude/java-random/blob/master/javarandom.py
# The contact email address for this would be... Corbin Simpson <MostAwesomeDude at gmail com>
...
"""
        ...
        bits = self.next(31)
        val = (bits % n)
        bitloop = (bits - val + (n-1))
        if bitloop & (1 << 31):
            bitloop -= (1 << 32)

        while (bitloop) < 0:	#! AHA!? Output the value of this when hitting val ==0
            bits = self.next(31)
            val = (bits % n)
            bitloop = (bits - val + (n-1))
            if bitloop & (1 << 31):
